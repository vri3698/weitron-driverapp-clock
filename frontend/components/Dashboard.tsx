...
export function Dashboard({
  ...
  return (
    <div className="min-h-dvh bg-[radial-gradient(circle_at_top,_rgba(99,102,241,0.25),_transparent_55%)] bg-slate-950 p-4 text-white">
      <div className="mx-auto flex max-w-md flex-col gap-4 rounded-[28px] border border-white/10 bg-slate-900/80 p-4 shadow-2xl shadow-black/30 backdrop-blur">
        <div className="flex items-start justify-between rounded-2xl bg-slate-800/80 p-4">
          <div>
          ...
          <button onClick={onLogout} className="rounded-full border border-slate-700 px-3 py-1.5 text-sm text-slate-300">
            Sign out
          </button>
        </div>

        <div className={`flex items-center gap-2 rounded-2xl px-3 py-2 text-sm ${isOnline ? 'bg-emerald-900/40 text-emerald-300' : 'bg-amber-900/40 text-amber-300'}`}>
          {isOnline ? <Wifi size={16} /> : <WifiOff size={16} />}
          {isOnline ? 'Online and ready' : 'Offline — will sync later'}
        </div>

        {statusMessage ? (
          <div className="flex items-center gap-2 rounded-2xl bg-emerald-900/40 px-3 py-2 text-sm text-emerald-300">
        ...
        {!setupComplete || !localStorage.getItem(setupDoneKey) ? (
          <div className="rounded-2xl border border-indigo-700/40 bg-indigo-950/40 p-4">
            <p className="mb-2 text-sm font-semibold text-indigo-300">Complete setup before first clock action</p>
            <div className="grid gap-2">
              {!locationReady ? (
                <button
                  onClick={() => void handleEnableLocation()}
                  className="flex items-center justify-center gap-2 rounded-2xl bg-slate-700/80 px-3 py-2.5 text-sm font-medium text-slate-200 hover:bg-slate-700"
                >
              ...
              {!notifReady && !showIOSInstallPrompt ? (
                <button
                  ...
                  className="flex items-center justify-center gap-2 rounded-2xl bg-slate-700/80 px-3 py-2.5 text-sm font-medium text-slate-200 hover:bg-slate-700 disabled:opacity-60"
                >
                  <Bell size={15} />
        ...
        {isOnBreak ? (
          <div className="rounded-2xl border border-amber-600/40 bg-amber-950/30 p-4">
            ...
            <p className="mb-3 text-3xl font-bold tracking-wider text-amber-200">{breakRemaining}</p>
            <p className="mb-3 text-xs text-amber-100/80">30-minute break timer is running.</p>
            <button
              onClick={onEndBreak}
              className="w-full rounded-2xl bg-indigo-600 px-3 py-2.5 text-sm font-semibold text-white transition hover:bg-indigo-500"
            >
              End break
            </button>
          </div>
          ...
          <div className="rounded-2xl border border-slate-800 bg-slate-800/70 p-3">
            ...
            {hasFinishedDay ? (
              <p className="rounded-xl bg-emerald-900/30 px-3 py-3 text-center text-sm text-emerald-300">
              ...
              <button
                ...
                className={`flex w-full items-center justify-center gap-2 rounded-2xl px-3 py-3 font-semibold transition disabled:cursor-not-allowed disabled:opacity-60 ${
                  nextAction === 'Clock In'
        ...
        {setupMsg ? (
          <p className="text-center text-xs text-slate-400">{setupMsg}</p>
        ) : null}
      </div>
    </div>
  );
